...
function ensureRenderer() {
  ...
  // createRenderer 创建渲染器，笔记22

  // 返回一个渲染器renderer对象
  // rendererOptions：const rendererOptions = extend({patchProp},nodeOps)
  // patchProp:处理元素的props、attribute、class、style、event事件等
  // nodeOps：处理dom节点，这个对象包含了各种原生dom操作方法
  // rendererOptions对象最终会传递到渲染器里面，里面各种方法最终会在页面渲染过程被调用

  // renderer渲染器的类型
  // 1.Renderer web端渲染器   2.HydrationRenderer服务器端渲染器
  // Hydration开头的函数为SSR服务器端渲染使用。
  // 所以renderer渲染器可以是web端渲染器，也可以是SSR渲染器，在浏览器中使用vue则确定为web端
}
